...
task main()
{
	...
	{	// obstacle scenario
		if (getUSDistance(US) <= 10)
		{
			...
			if((getColorName(CS) == colorBlue) || (getColorName(CS) == colorBlack))
			{
				moveMotorTarget(leftMotor, -720, 50);  //vars: motor index, position and speed
				moveMotorTarget(rightMotor, -720, 50);
			}

			// obstacle in green line
			else if (getColorName(CS) == colorGreen)
				{
					while (getUSDistance < 5)
						{
