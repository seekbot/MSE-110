...
void adjust(){
	while(getUSDistance(US)!= adjustDist){
		if(getUSDistance(US)>adjustDist){ // adjust back by moving forward
			setMotorSpeed(leftWheel, -20);
			setMotorSpeed(rightWheel, -20);
		}
		if (getUSDistance(US)<adjustDist){ // adjust back by moving backward
			setMotorSpeed(leftWheel, 20);
			setMotorSpeed(rightWheel, 20);
		}
	}
...
void back2Start(){
	...
	for(int i = 0; i < wayBackIndex; i++){
		while(robotDirection != wayBack[i]){ //make robot face the correct direction
			turnRight();
		}
...
void turnRight(){
	// physical robot
	setMotorTarget(leftWheel,-195, speed); // Around 355 deg wheel rotation = 90 deg bot turn
	setMotorTarget(rightWheel,195,speed);
...
void turnLeft(){
	// physical robot
	setMotorTarget(leftWheel,200, speed); // Around 355 deg wheel rotation = 90 deg bot turn
	setMotorTarget(rightWheel,-200,speed);
